...
factor factor::log() const {
	factor ret(s);
	std::transform(f.begin(), f.end(), ret.f.begin(), (double(*)(double))std::log);
	return ret;
}

factor factor::exp() const {
	factor ret(s);
	std::transform(f.begin(), f.end(), ret.f.begin(), (double(*)(double))std::exp);
	return ret;
}
